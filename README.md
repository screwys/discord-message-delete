...
Emoji rules accept a standard shortcode such as `:thumbsup:`, a custom emoji name such as `:team_badge:`, the Unicode emoji itself, or a custom emoji mention copied from Discord such as `<:party:123456789012345678>`. Known standard shortcodes are stored as Unicode. Other shortcode names match custom emoji by name; custom mentions match by ID, which remains stable if the emoji is renamed. Emoji message rules inspect only authored message content, not generated embed metadata or link previews.
